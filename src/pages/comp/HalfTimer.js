...
const HalfTimer = ({route}) => {
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const url = `${url}/chat-page/?libid=${id}&user_id=${userId['user_id']}&sign=${res['sign']}&url=add-half-time`;
      console.log(url)
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${url}/half-day-student/${id}/`, {
        headers: {
          Authorization: 'Bearer ' + String(user.access),
        },
      });
